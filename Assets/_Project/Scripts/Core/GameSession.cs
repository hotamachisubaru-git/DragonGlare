    ...
    public class GameSession : MonoBehaviour
    {
        ...
        public IReadOnlyList<BattleSelectionEntry> GetActiveBattleSelectionEntries()
        {
            return BattleFlowState switch
            {
                ...
                BattleFlowState.ItemSelection => Player.Inventory.Where(i => i.Item is ConsumableDefinition).Select(i => new BattleSelectionEntry { Label = i.Item.Name, Detail = $"x{i.Quantity}", Consumable = (ConsumableDefinition)i.Item }).ToList(),
                BattleFlowState.EquipmentSelection => Player.Inventory.Where(i => i.Item is IEquipmentDefinition).Select(i => new BattleSelectionEntry { Label = i.Item.Name, Detail = string.Empty, Equipment = (IEquipmentDefinition)i.Item }).ToList(),
