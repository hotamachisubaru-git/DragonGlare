...
public sealed class FieldEventService
{
    ...
    private static IEnumerable<string> ApplyTreasureReward(PlayerProgress player, FieldEventDefinition fieldEvent, UiLanguage language)
    {
        ...
        {
            yield return Text(language, "縺励°縺・縺ｪ縺ｫ繧・縺ｯ縺・▲縺ｦ縺・↑縺九▲縺溘・, "But it was empty.");
            yield break;
        }
        ...
        if (reward.HasItem)
        {
            ...
            var quantitySuffix = reward.ItemQuantity > 1
                ? Text(language, $"ﾃ養reward.ItemQuantity}", $" x{reward.ItemQuantity}")
                : string.Empty;
            yield return Text(language, $"{itemName}{quantitySuffix}繧・縺ｦ縺ｫ縺・ｌ縺滂ｼ・, $"Received {itemName}{quantitySuffix}!");
        }
        ...
        if (reward.HasGold)
        {
            ...
            yield return gainedGold > 0
                ? Text(language, $"{gainedGold}G繧・縺ｦ縺ｫ縺・ｌ縺滂ｼ・, $"Received {gainedGold}G!")
                : Text(language, "縺薙ｌ縺・§繧・≧ 縺翫°縺ｭ繧・繧ゅ※縺ｪ縺・・, "You cannot carry any more gold.");
        }
    }
    ...
    private static string GetPlayerName(PlayerProgress player)
    {
        ...
        return player.Language == UiLanguage.English ? "adventurer" : "縺ｼ縺・￠繧薙＠繧・;
    }
